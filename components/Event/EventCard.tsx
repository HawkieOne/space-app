...
export default function EventCard({ event, onClick }: EventCardProps) {
  return (
    <Card onClick={onClick}>
      <img
        className="h-full w-80 self-center rounded-l-lg object-cover shadow-2xl"
      ...
      <div className="flex w-full flex-col justify-start space-y-2">
        ...
        <div className="flex space-x-2">
          <div className="flex items-center rounded-full bg-yellow-500 px-2 py-1 text-xs">
            {event?.location}
          </div>
          <div className="flex items-center rounded-full bg-sky-500 px-2 py-1 text-xs">
            {event?.launches[0]?.launch_service_provider.name}
            {event?.location}
          </div>
          <div className="flex items-center rounded-full bg-lime-500 px-2 py-1 text-xs">
            {event?.launches[0]?.name}
          </div>
        </div>
        <span className="text-grey-darkest text-5xl">{event?.name}</span>
